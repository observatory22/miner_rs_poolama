...
// m can be passed as separate regs (m0..m15) or you can load them from memory
// first. This keeps *state* entirely in registers via references.
__device__ __forceinline__ void g_round_regs(
    uint32_t &s0, uint32_t &s1, uint32_t &s2, uint32_t &s3, uint32_t &s4,
    uint32_t &s5, uint32_t &s6, uint32_t &s7, uint32_t &s8, uint32_t &s9,
    uint32_t &s10, uint32_t &s11, uint32_t &s12, uint32_t &s13, uint32_t &s14,
    uint32_t &s15, const uint32_t m0, const uint32_t m1, const uint32_t m2,
    const uint32_t m3, const uint32_t m4, const uint32_t m5, const uint32_t m6,
    const uint32_t m7, const uint32_t m8, const uint32_t m9, const uint32_t m10,
    const uint32_t m11, const uint32_t m12, const uint32_t m13,
    const uint32_t m14, const uint32_t m15) {
  // Mix the columns.
...
g_compress(const u32 *__restrict__ chaining_value, // cv[8]
           const u32 *__restrict__ block_words,    // m[16]
           u32 counter, u32 block_len, u32 flags,
           u32 *__restrict__ state_out) // writes v[16]
{
  ...
  u32 s0 = cv0, s1 = cv1, s2 = cv2, s3 = cv3, s4 = cv4, s5 = cv5, s6 = cv6,
      s7 = cv7;
  ...
  // Round 0: identity
  g_round_regs(s0, s1, s2, s3, s4, s5, s6, s7, s8, s9, s10, s11, s12, s13, s14,
               s15, m0, m1, m2, m3, m4, m5, m6, m7, m8, m9, m10, m11, m12, m13,
               m14, m15);

  // Round 1: perm^1 = [2,6,3,10,7,0,4,13,1,11,12,5,9,14,15,8]
  g_round_regs(s0, s1, s2, s3, s4, s5, s6, s7, s8, s9, s10, s11, s12, s13, s14,
               s15, m2, m6, m3, m10, m7, m0, m4, m13, m1, m11, m12, m5, m9, m14,
               m15, m8);

  // Round 2: perm^2 = [3,4,10,12,13,2,7,14,6,5,9,0,11,15,8,1]
  g_round_regs(s0, s1, s2, s3, s4, s5, s6, s7, s8, s9, s10, s11, s12, s13, s14,
               s15, m3, m4, m10, m12, m13, m2, m7, m14, m6, m5, m9, m0, m11,
               m15, m8, m1);

  // Round 3: perm^3 = [10,7,12,9,14,3,13,15,4,0,11,2,5,8,1,6]
  g_round_regs(s0, s1, s2, s3, s4, s5, s6, s7, s8, s9, s10, s11, s12, s13, s14,
               s15, m10, m7, m12, m9, m14, m3, m13, m15, m4, m0, m11, m2, m5,
               m8, m1, m6);

  // Round 4: perm^4 = [12,13,9,11,15,10,14,8,7,2,5,3,0,1,6,4]
  g_round_regs(s0, s1, s2, s3, s4, s5, s6, s7, s8, s9, s10, s11, s12, s13, s14,
               s15, m12, m13, m9, m11, m15, m10, m14, m8, m7, m2, m5, m3, m0,
               m1, m6, m4);

  // Round 5: perm^5 = [9,14,11,5,8,12,15,1,13,3,0,10,2,6,4,7]
  g_round_regs(s0, s1, s2, s3, s4, s5, s6, s7, s8, s9, s10, s11, s12, s13, s14,
               s15, m9, m14, m11, m5, m8, m12, m15, m1, m13, m3, m0, m10, m2,
               m6, m4, m7);

  // Round 6: perm^6 = [11,15,5,0,1,9,8,6,14,10,2,12,3,4,7,13]
  g_round_regs(s0, s1, s2, s3, s4, s5, s6, s7, s8, s9, s10, s11, s12, s13, s14,
               s15, m11, m15, m5, m0, m1, m9, m8, m6, m14, m10, m2, m12, m3, m4,
               m7, m13);
...
#else
__constant__ const int g_MSG_PERMUTATION[] = {2, 6,  3,  10, 7, 0,  4,  13,
                                              1, 11, 12, 5,  9, 14, 15, 8};

__device__ __forceinline__ u32 g_rotr(u32 value, int shift) {
  // OPTIMIZATION: Use fast bit rotation with compiler intrinsics for lower
  // precision/higher speed
  return __funnelshift_r(value, value, shift);
}

__device__ __forceinline__ void g_g(u32 state[16], u32 a, u32 b, u32 c, u32 d,
                                    u32 mx, u32 my) {
  // OPTIMIZATION: Fast arithmetic with reduced precision (CPU verification will
  // catch errors) Use fast unchecked arithmetic - overflow is acceptable for
  // speed
...
__device__ __forceinline__ void g_compress(const u32 *chaining_value,
                                           const u32 *block_words, u64 counter,
...
extern "C" __global__ void compress(u32 *__restrict__ chaining_value, // cv[8]
                                    ...
                                    u32 *__restrict__ state_out) // writes v[16]
{
  ...
  if (ROW == 0 && COL == 0) {
    g_compress(chaining_value, block_words, counter & 0xffffffff, block_len,
               flags, state_out);
...
__device__ inline void xof_emit_words(u32 blk, const u32 root[8],
                                      const u32 precv[8],
                                      const u32 last_words[16], u32 last_len,
                                      u32 dstw[16]) {
  u32 out[16];
  ...
  // g_compress writes 16 words to state; its low half is already lo^hi (root
  // CV), high half is the raw hi (no feed-forward).
...
__device__ __forceinline__ void store_be64(uint8_t *dst, u64 x) {
#pragma unroll
  for (int i = 0; i < 8; ++i)
    dst[7 - i] = (uint8_t)(x >> (8 * i));
}
// Compute root + pre-final CV + last words from a 240B seed
__device__ __forceinline__ void
compute_root_from_seed240(const uint8_t *__restrict__ seed240,
...
//   nonce_start : starting 64-bit nonce (little-endian written into bytes
//   232..239) nonce_count : number of seeds (one 16x16 output per nonce) d_C :
//   outputs [nonce_count][16][16] int32
// -----------------------------------------------------------------------------
#ifndef TILE_K
#define TILE_K 256
#endif

__device__ void fused_blake3_hash_and_detect(
    ...
    uint32_t *__restrict__ d_found_u32_at_228, uint64_t seed_n,

    unsigned long long *__restrict__ ring_nonces, // u64[cap]
    int ring_cap,                                 // capacity
    int *ring_flags,                              // i32[cap], 0=empty, 1=full
    unsigned long long
        *__restrict__ ring_tail // monotonically increasing reservation counter
) {
...
#pragma unroll
  ...
  u32 final_hash[8];
  {
    ...
    *reinterpret_cast<uint4 *>(parent_in) =
        *reinterpret_cast<const uint4 *>(cv0);
    *reinterpret_cast<uint4 *>(parent_in + 4) =
        *reinterpret_cast<const uint4 *>(cv0 + 4);
    *reinterpret_cast<uint4 *>(parent_in + 8) =
        *reinterpret_cast<const uint4 *>(cv1);
    *reinterpret_cast<uint4 *>(parent_in + 12) =
        *reinterpret_cast<const uint4 *>(cv1 + 4);

    u32 cv[8];
    // OPTIMIZATION: Vectorized IV initialization
    *reinterpret_cast<uint4 *>(cv) = *reinterpret_cast<const uint4 *>(g_IV);
    *reinterpret_cast<uint4 *>(cv + 4) =
        *reinterpret_cast<const uint4 *>(g_IV + 4);
    ...
    *reinterpret_cast<uint4 *>(final_hash) =
        *reinterpret_cast<const uint4 *>(st);
    *reinterpret_cast<uint4 *>(final_hash + 4) =
        *reinterpret_cast<const uint4 *>(st + 4);
  }
  ...
  if ((h[0] == 0x00) && (h[1] == 0x00) && ((h[2] & 0xF0) == 0x00)) {
    ...
    *d_found_u32_at_228 = *p228;
    ...
    // If you prefer "never block" & allow loss, replace the spin with a
    // try-then-drop.
...
__device__ __forceinline__ int pack4_sub128(uint8_t b0, uint8_t b1, uint8_t b2,
                                            uint8_t b3) {
...
__device__ __forceinline__ int pack4_i8(uint8_t b0, uint8_t b1, uint8_t b2,
                                        uint8_t b3) {
...
extern "C" __global__ __launch_bounds__(256, 4) void solve_nonce_range_fused(
    const uint8_t *__restrict__ d_prefix232, // 232 bytes
    unsigned long long *d_iter_count, u64 nonce_start, u32 nonce_count,
...
#pragma unroll
  ...
  // Layout: [A_packed (16 rows × TILE_K/4 ints)] [PAD] [B_packed (TILE_K/4
  // groups × 16 cols ints)] Use a small pad to reduce bank conflicts between
  // the two regions.
  int32_t *As4 = reinterpret_cast<int32_t *>(smem);
  int32_t *Bs4 = reinterpret_cast<int32_t *>(
      smem + (size_t)16 * (TILE_K / 4) * sizeof(int32_t) + 64);
...
#pragma unroll
      for (int t = 0; t < 232; ++t)
        sh_seed[t] = sh_prefix[t];

      const u64 nonce = nonce_start + (u64)seed;
      // const u64 nonce_swapped = __bswap_constant_64(nonce);
      // store_le64(&sh_seed[232], nonce);
      store_be64(&sh_seed[232], nonce);

      compute_root_from_seed240(sh_seed, sh_root, sh_precv, sh_lwords,
                                &sh_llen);
    }
    __syncthreads();
    ...
    for (int k0 = 0; k0 < K; k0 += TILE_K) {
      ...
      for (int linear_idx = thread_id; linear_idx < 16 * a_blocks_per_row;
           linear_idx += total_threads) {
        const int ri = linear_idx / a_blocks_per_row; // row 0..15
        const int rb =
            linear_idx % a_blocks_per_row; // 64B block index within the row
...
#pragma unroll
      ...
      // =========================================================================
      // 2) Produce & PRE-PACK B: (tile × 16) but store as groups along k for
      // each col j
...
#pragma unroll
      ...
      // =========================================================================
      // 3) Compute:  One DP4A per 4-k group. Also sum_b via DP4A with
      // 0x01010101.
      // =========================================================================
    ...
    fused_blake3_hash_and_detect(
        sh_seed, tileC, &d_found_nonce, &d_found_u32_at_228, nonce_start + seed,
